...
module.exports = function (sockPath, key, keyType, data, cb) {
        ...
        */
        if (keys === undefined) {
          ...
          if (!key) {
            ...
            if (++count === 4) {
              count = 0;
              if (comlen > 0) comment = comlen;else {
                key = undefined;
                comment = false;
              }
              comlen = 0;
        ...
        if (isSigning) error = new Error('Agent unable to sign data');else error = new Error('Unable to retrieve list of keys from agent');
        sock.removeAllListeners('data');
        return sock.destroy();
      }
    }
  ...
  function onclose() {
    if (error) cb(error);else if (isSigning && !sig || !isSigning && !keys) cb(new Error('Unexpected disconnection from agent'));else if (isSigning && sig) cb(undefined, sig);else if (!isSigning && keys) cb(undefined, keys);
  }
    ...
    if (sockPath === 'pageant') {
      ...
      fs.readFile(sockPath, function readCygsocket(err, data) {
        if (err) {
          ...
          cp.exec('cygpath -w "' + sockPath + '"', function (err, stdout, stderr) {
        ...
        if (m = RE_CYGWIN_SOCK.exec(data.toString('ascii'))) {
          ...
          for (i = 0, j = 0; j < 32; ++i, j += 2) secretbuf[i] = parseInt(secret.substring(j, j + 2), 16);

          // convert to host order (always LE for Windows)
          for (i = 0; i < 16; i += 4) secretbuf.writeUInt32LE(secretbuf.readUInt32BE(i, true), i, true);
          ...
          tryConnect();
        } else cb(new Error('Malformed cygwin unix socket file'));
      });
      return;
    }
  ...
  function addSockListeners() {
    if (!accept && !reject) {
      ...
      sock.once('connect', function () {
        ...
        chan.once('end', onDone).once('close', onDone).on('data', function (data) {
          sock.write(data);
        });
...
if (process.platform === 'win32') {
  ...
  PageantSock.prototype.write = function (buf) {
    if (this.buffer === null) this.buffer = buf;else {
    ...
    if (this.buffer.length > 4 + len) this.buffer = this.buffer.slice(4 + len);else this.buffer = null;
